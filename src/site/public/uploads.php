...
$importFormats = @json_decode(@file_get_contents(__DIR__ . '/import-formats.json'), true);

$supportedFormats = ['lossless-image', 'lossy-image', 'patch'];
if ($importFormats) {
  $filteredFormats = [];
  foreach ($importFormats as $key => $formatConfig) {
    if (in_array($key, $supportedFormats)) {
      $filteredFormats[$key] = $formatConfig;
    }
  }
  $importFormats = $filteredFormats;
}
