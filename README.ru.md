...
- [x] загрузка в черновики Lossy-файлов для постов из раздела `Extras`;
- [x] [варианты соотношения сторон для постов одного типа, объединение `wallpaper` и `wallpaper-v`](https://github.com/dehero/mwscr/issues/442);
...
- [ ] тип постов `comparison` для раздела `extras` с различными сравнениями - например, графики OpenMW и Vanilla, для
      ремейков скриншотов из оригинальной игры и т.п.;
...
- [ ] кнопка "Открыть случайный пост", ведущая к случайной странице поста;
- [ ] список локаций, ещё не отмеченных в постах;
- [ ] кнопка для выбора случайной локации (опционально - только не посещённые) с консольной командой для перехода туда;
