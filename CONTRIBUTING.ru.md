...
> [!WARNING]  
> Этот документ частично устарел. Для получения более актуальной информации перейдите на
> [страницу справки на сайте проекта](https://mwscr.dehero.site/help/).
...
Чтобы попросить авторов проекта сделать определенный скриншот или видео, оставьте сообщение в свободной форме любым
удобным способом. Проще всего написать комментарий или личное сообщение администратору в любом из аккаунтов проекта.

Другой вариант - воспользоваться [диалогом](https://mwscr.dehero.site/#post-request/drafts) на сайте проекта, чтобы
затем отправить свой запрос в виде патча. Ваш запрос будет добавлен в [черновики](./drafts.md) и останется там в
ожидании подходящего материала.
...
Перейдите на страницу поста или нажмите правую кнопку мыши (сделайте долгое нажатие) на посте, чтобы открыть контекстное
меню. Нажмите кнопку `Edit`, чтобы заполнить поля поста.

#### Тип, формат и содержание

В зависимости от содержания поста и его характеристик, выбирается подходящий тип поста:

| Тип           | Форматы                     | Файлов |    Длит. | Дополнительные требования                                                                     |
| ------------- | :-------------------------- | -----: | -------: | --------------------------------------------------------------------------------------------- |
| `shot`        | `1:1` `3:2`                 |      1 |          |                                                                                               |
| `compilation` | `1:1` `3:2` `16:9` `9:19.5` |      4 |          | Все кадры должны быть ранее опубликованы в разделе [Posts](https://mwscr.dehero.site/posts/). |
| `wallpaper`   | `16:9` `9:19.5`             |      1 |          |                                                                                               |
| `clip`        | `1:1` `9:16`                |      1 |  ≤ 1 мин |                                                                                               |
| `video`       | `16:9`                      |      1 | ≤ 60 мин |                                                                                               |

Набор форматов ограничен, каждый из них имеет минимальный и рекомендуемый размер:

| Формат   | Рекомендуемый размер | Минимальный размер |
| :------- | :------------------: | :----------------: |
...
Содержание подстраивается под выбранный тип и формат поста, а все отвергнутые файлы попадают в корзину поста.
...
Для постов типа [Compilation](https://mwscr.dehero.site/help/compilation/) перечисляются авторы всех кадров в подборке.
