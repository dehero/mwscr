...
Ваш запрос будет добавлен во [входящие](docs/inbox/index.md) и останется там, дожидаясь подходящий материал.
...
- Нажать [`📍 Locate`](https://github.com/dehero/mwscr/issues/new?labels=location&template=location.yml&title=POST_ID) в
  описании поста, чтобы предложить локацию через Issues.
...
[предлагаемые посты](docs/inbox/index.md). При возникновении вопросов пишите
[администратору](docs/contributors.md#dehero) любым удобным способом.
...
этого используется
[заявка на слияние](https://github.com/dehero/mwscr/issues/new?labels=merging&template=merging.yml&title=POST_ID).
...
Обработанные версии будут добавлены в [предлагаемые посты](docs/inbox/index.md) при следующем импорте файлов из
хранилища.

### Подготовка поста

C помощью
[заявки на редактирование](https://github.com/dehero/mwscr/issues/new?labels=editing&template=editing.yml&title=POST_ID)
...
| `drawing`  |  1:1   |      2 |  1080x1080  |   800x800   |          | Второй кадр должен быть ранее опубликован с типом `shot`. |
...
Указывается участник проекта, предложивший исходную работу к публикации. Для поста типа `drawing` первым указывается
...
Указывается игровая локация согласно [списку локаций](data/locations.lst). Если точное местоположение определить
...
[заявка на ревью](https://github.com/dehero/mwscr/issues/new?labels=review&template=review.yml&title=POST_ID).
